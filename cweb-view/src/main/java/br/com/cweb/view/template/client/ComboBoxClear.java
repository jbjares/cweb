...
/**
 * @author Joao Bosco Jares A. Chaves joao.cahves@pdcase.com.br
 * @version 1.0
 * 
 */
public class ComboBoxClear<D extends ModelData> extends TriggerFieldClear<D> implements SelectionProvider<D> {
	 /**
	...
	public ComboBoxClear() {
	  ...
	  protected void initList() {
	    ...
	    list = new LayoutContainer() {
	      ...
	      protected void doAttachChildren() {
	        ...
	        ComponentHelper.doAttach(ComboBoxClear.this.pageTb);
	      ...
	      protected void doDetachChildren() {
	        ...
	        ComponentHelper.doDetach(ComboBoxClear.this.pageTb);
	      }
	  ...
	  private void trocarBotaoBuscaLimpar(boolean isTrocar) {
		  if(isTrocar){
			  removeStyleName("x-trigger-wrap-focus");
			  clearTrigger = new El(DOM.createImg());
			  clearTrigger.dom.setClassName("x-form-clear-trigger " + clearTriggerStyle);
			  clearTrigger.dom.setPropertyString("src", GXT.BLANK_IMAGE_URL);
			  el().appendChild(input.dom);
			  el().appendChild(clearTrigger.dom);				  
		  }else{
	...
	protected void onClearTriggerClick(ComponentEvent ce) {
	    super.onTriggerClick(ce);
	    clearSelections();
	    trocarBotaoBuscaLimpar(false);
	    getInputEl().focus();
	  }
